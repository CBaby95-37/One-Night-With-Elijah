...
const hallFloor = new THREE.Mesh(new THREE.PlaneGeometry(4, 21), woodMat); 
hallFloor.rotation.x = -Math.PI / 2; hallFloor.position.set(-7.5, 0.01, 7.5);
...
fakeDoor1.position.set(-9.5, 2, 4); scene.add(fakeDoor1);
...
for(let i=0; i<30; i++) {
    ...
    fencePost.position.set(15, 1.5, -10 + (i*2)); 
...
const hallLight = new THREE.PointLight(0x444455, 0.6, 20); 
hallLight.position.set(-7.5, 5, 8); scene.add(hallLight);
